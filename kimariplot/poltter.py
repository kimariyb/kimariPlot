...
def plot_line_path(ax, x, y, labels, color, style):
    """
    ...
    for i, (x_new, y_new) in enumerate(zip(x, y)):
        ...
        # 显示数字
        ax.text(x_new, y_new + 0.5, f"{y_new:.1f}", ha='center', va='bottom', fontsize=8, fontweight='medium')
        # 绘制 label
        ax.text(x_new, y_new - 1.25, labels[i], ha='center', va='top', fontsize=8.5, fontweight='bold')
...
def plot_all_line_paths(data_list, dpi, size, font, output_type):
    ...
    for plot_data in reversed_data_list:
        ...
        x = plot_data.get_x()
        y = plot_data.get_y()
        plot_line_path(ax, x, y, labels, color, style)
